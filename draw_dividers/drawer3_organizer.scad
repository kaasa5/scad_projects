...
module parts(part) {
    if (part == "connector_all") {
        ...
        //Left side
        left_dimx = width_top/2;
        left_dimy = (width_top/2)+border_overhang;
        translate([left_dimx,left_dimy,0])
        connector_corner(round_outside=false, round_inside=true, border=true);
        translate([left_dimx,left_dimy+(0.5*connector_length),0]){
            rotate([0,0,180]) 
            divider(border=true, length=150);
        }
        translate([left_dimx,left_dimy+connector_length+150,0]){
            rotate([0,0,180]) 
            connector_t(round=true, border=true);
        }
        translate([left_dimx,left_dimy+(0.5*connector_length)+185,0]){
            rotate([0,0,180]) 
            divider(border=true, length=185);
        }
        translate([left_dimx,left_dimy+connector_length+370,0]){
            rotate([0,0,180]) 
            connector_t(round=true, border=true);
        }
        translate([left_dimx,left_dimy+(connector_length*1.5)+370]){
            rotate([0,0,180]) 
            divider(border=true, length=62);
        }
        translate([left_dimx+border_overhang,left_dimy+(connector_length*1.5)+447,0]){
            rotate([0,0,270]) 
            connector_corner(round_outside=false, round_inside=true, border=true);
        }
        //back
        back_dimy = (connector_length*1.5)+(width_top/2)+border_overhang+447;
        back_dimx = border_overhang+(width_top/2)+(0.5*connector_length);
        translate([back_dimx,back_dimy,0]){
            rotate([0,0,90]) 
            divider(border=true, length=216);
        }
        translate([back_dimx+(connector_length/2)+216,back_dimy,0]){
            rotate([0,0,90]) 
            connector_t(round=true, border=true);
        }
        translate([back_dimx+connector_length+216,back_dimy,0]){
            rotate([0,0,90]) 
            divider(border=true, length=216);
        }
        //right
        right_dim = border_overhang+width_top+connector_length+447.5;
        ...
        translate([border_overhang+right_dim,0.5*connector_length+width_top/2+border_overhang+370,0]){
            rotate([0,0,0]) 
            divider(border=true, length=185);
        }
